...
#include "Tools/controllerGlobals.h"
...
void SDONController::recvControlInfo(
...
void SDONController::launchControlMetricsProducer() {
  LOG(LOG_CONTROLLER) <<
  "Creating SDONController::metrics producer thread";

  std::thread metricsProducer(
      &SDONController::collectControlMetricsForAWindowTime(), this);
...
void SDONController::collectControlMetricsForAWindowTime() {
  ...
  while (!g_stop.load()) {
    ...
    while (endWindowTime_s > std::chrono::steady_clock::now()) {
      recvControlInfo(metrics, directives);
...
void SDONController::consumeControlMetrics() {
  g_startedThread++; //??? aqui?
  while (!g_stop.load()) {
...
void SDONController::processControlMetrics() {
  //to avoid data race over the metris/directives list.
  //the producer maight be quiquer than the consumer to produce more
  //data.
  std::vector<std::map<NetworkMetricsControlCode, value_t>> metrics =
      m_receivedControlMetrics;
  std::vector<std::map<DirectiveControlCode, value_t>> directives =
      m_receivedControlDirectives;
  for(std::map<NetworkMetricsControlCode, value_t>metric : metrics){
